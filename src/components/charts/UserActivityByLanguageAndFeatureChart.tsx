...
export default function UserActivityByLanguageAndFeatureChart({
  ...
  const groupedAndSorted = useMemo(() => {
    ...
    const sortedLanguages = Object.keys(grouped).sort((a, b) => {
      ...
      const totalA = grouped[a].reduce((sum, i) => sum + i.code_generation_activity_count, 0);
      const totalB = grouped[b].reduce((sum, i) => sum + i.code_generation_activity_count, 0);
      return totalB - totalA;
    });

    return { grouped, sortedLanguages };
  }, [languageFeatureAggregates]);
